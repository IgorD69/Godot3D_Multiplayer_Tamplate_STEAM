...
#@onready var tray_anim: AnimationPlayer = $StaticBody/AnimationPlayer
@onready var tray_anim: AnimationPlayer = $"../../Sertar/StaticBody/AnimationPlayer"
...
func play_sfx(index: int):
	print("Încerc să redau sunetul la indexul: ", index) # Debug
	if index < sfx_playlist.size() and sfx_playlist[index] != null:
		sfx_player.stream = sfx_playlist[index]
		sfx_player.play()
		print("Sunetul rulează: ", sfx_player.stream.resource_path)
	else:
		print("Eroare: Index invalid sau stream gol în playlist!")
